...
GuiPage_Search.keyDown = function() {
	...
	switch(keyCode) {	
		...
		case tvKey.KEY_YELLOW:	
			...
			break;				
		case tvKey.KEY_BLUE:	
			GuiMusicPlayer.showMusicPlayer("GuiPage_Search",this.playItems[this.selectedItem2]+this.ItemData.SearchHints[this.selectedItem].ItemId,"guiMusic_TableTd SelectedButton");
			break;	
		case tvKey.KEY_EXIT:
			alert ("EXIT KEY");
			widgetAPI.sendExitEvent();
...
GuiPage_Search.openMenu = function(hasData) {
	if (hasData) {
		...
		this.selectedItem2 = 0;
		GuiMainMenu.requested("GuiPage_Search",this.playItems[this.selectedItem2]+this.ItemData.SearchHints[this.selectedItem].ItemId,"guiMusic_TableTd SelectedButton");
	} else {
